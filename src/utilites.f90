...
SUBROUTINE writeVtk_field (Npoint,  sdx, sdy, sdz, nCells, delta, Uaf, Jaf,Jvx,Jvy,Jvz, Bfex, Bfey, Bfez,  files)
! This program generates a .vtk file for fields 
USE m_vxc2data
IMPLICIT NONE

INTEGER,      INTENT(IN):: Npoint,  sdx,sdy,sdz
REAL(8),      INTENT(IN):: Uaf(*), Jaf(*), Jvx(*), Jvy(*), Jvz(*)
REAL(8),      INTENT(IN):: Bfex(sdx,sdy,sdz), Bfey(sdx,sdy,sdz), Bfez(sdx,sdy,sdz),  delta(3)
CHARACTER(*), INTENT(IN):: files
CHARACTER (LEN = 1)  ci
CHARACTER (LEN = 4)  ch_sd
CHARACTER (LEN = 30) buf1
INTEGER:: i,j,k,m, n,kdz, nCells, ios, n_new 
REAL(8):: s,sxm,sym,szm 
...
ELSE
    WRITE(n_new) "VECTORS "//"field_SOURCE"//" float"//new_line(ci)
    m=0
    s=0.07957747154594766788444d7 ! = 1/mu0
    DO k = 1,sdz;  DO j = 1,sdy;  DO i = 1,sdx
        m = m+1
        write(n_new) real( s*Jaf(m),4 ), real( s*Jaf(nCells+m),4  ), real(s*Jaf(2*nCells+m),4)
    ENDDO;  ENDDO;  ENDDO
...
do k=1,sdz;  do j=1,sdy;  do i=1,sdx  
     write(n_new) real(Bfex(i,j,k),4), real(Bfey(i,j,k),4), real(Bfez(i,j,k),4)
enddo; enddo; enddo; 
