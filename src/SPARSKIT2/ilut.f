...
c-----------------------------------------------------------
       ...
       real*8 s,tl
          ju0 = n+2
          jlu(1) = ju0
c initialize work vector to zeros
       do  i=1, n
            iw(i) = 0
       enddo
c
...
c-------------------------------------------------------------
20     ro = dnrm2(n, vv, 1)
  ! 20    ro = norm2(vv(1:n,1))
c=================================================
c       if (iout .gt. 0 .and. its .eq. 0) write(iout, 199) its, ro
c========================
...
c=========================================================
c       if (iout .gt. 0) write(iout, 199) its, ro
c===========================================================
...
c  Discussion:
c
c    This routine multiplies a matrix by a vector using the dot product form.
...
c  Parameters:
c
c    Input, integer ( kind = 4 ) N, the row dimension of the matrix.
c
c    Input, real X(*), and array of length equal to the column dimension 
c    of A.
c
c    Input, real A(*), integer ( kind = 4 ) JA(*), IA(NROW+1), the matrix in CSR
